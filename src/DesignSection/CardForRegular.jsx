...
const CardForRegular = ({ scrollToTop }) => {
  ...
  const { selectRate, PriceRange, PriceOrder, ForSearching, setRentData } =
  ...
  useEffect(() => {
    ...
    if (PriceRange && PriceRange.length === 2) {
      filtered = filtered.filter(
        (element) =>
          element.price >= PriceRange[0] && element.price <= PriceRange[1]
      );
    }

    if (selectRate && selectRate.length > 0) {
      filtered = filtered.filter((element) =>
        selectRate.includes(element.rateing)
      );
    }

    if (ForSearching) {
      filtered = filtered.filter((element) =>
        element.description.toLowerCase().includes(ForSearching.toLowerCase())
      );
    }
              ...
              <div className="relative group">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gray-800 opacity-0 group-hover:opacity-100 transition-transform transform translate-y-full group-hover:translate-y-0 mb-14">
                  <div className="px-6 py-4 text-gray-400 flex gap-8">
                    ...
                    >
                      More Info
                      <span className="text-red-800">{card.rateing}</span>⭐
                    </button>
                  </div>
                </div>
                <div className="flex flex-col justify-center items-center h-16">
                  <h1 className="text-black font-serif uppercase">
                    {card.description}
                  </h1>
                  <h1 className="text-lg font-semibold text-blue-600 font-serif">
                    Price: <span className="text-gray-800">₹{card.price}</span>
                  </h1>
                </div>
              </div>
            </div>
